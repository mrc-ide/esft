...
Max of hospital demand for the dates inclusive in the week specified. Hospital demand is the number of people who would be using a hospital bed given enough healthcare capacity.
...
Max of ICU demand for the dates inclusive in the week specified. ICU demand is the number of people who would be using an ICU bed given enough healthcare capacity
...
Sum of hospital incidence for the dates included in the week specified. Hospital incidence is the number of people who would be getting a hospital bed at the current time.
...
Sum of ICU incidence for the dates included in the week specified. ICU incidence is the number of people who would be getting an ICU bed at the current time.
...
Cumulative infections for the dates included in the week specified, up until the end date of the week specified ($t_{n}$).
...
The minimum of crit_patients_nocap (uncapped critical patients, nr. 3) and the number of beds allocated to critical COVID-19 patients. $$
\text{Critical beds in use} = \min(\text{Critical Patients Uncapped}, n_{\text{Critical COVID-19 beds}})
$$

8.  *sev_beds_inuse*

The minimum of sev_patients_nocap (uncapped severe patients, nr. 4) and the number of beds allocated to severe COVID-19 patients.
...
Sum of severe and critical beds in use (nr. 9) divided by the number of hospital beds per care unit, specified in the parameters.
...
Cumulative removed critical cases are calculated weekly by taking the difference between the cumulative critical cases (cum_critical_cases) and the uncapped critical patients (crit_patients_nocap, nr. 3).

$$
\text{Cumulative Removed}(Cases_{critical}) = \text{Cumulative}(Cases_{critical}) - 
\text{Critical Patients Uncapped} 
$$ Then you find the removed critical patients by taking the difference between each week's cumulative removed critical cases and the cumulative removed critical cases from the week before.
...
Cumulative removed severe cases are calculated weekly by taking the difference between the cumulative severe cases (cum_severe_cases) and the uncapped severe patients (sev_patients_nocap, nr. 4).
...
Then you find the removed severe patients by taking the difference between each week's cumulative removed severe cases and the cumulative removed severe cases from the week before.
...
Sum of new severe and critical cases multiplied by the number of tests for diagnosis for severe or critical cases.
...
Number of beds (or beds allocated to covid) times the number of hygienists per bed (nr. 1q).
...
Percent HCWs allocated to treating COVID-19 (nr. 16) multiplied by the number of HCWs available.
...
This is the percent of HCWs dedicated to screening COVID-19 cases (nr. 14) multiplied by the number of HCWs available.
...
The percent of HCWs dedicated to treating COVID-19 are found by multiplying the ratio of inpatient HCWs to outpatient HCWS (nr. 19) by one minus the percent of HCWs allocated to non-COVID-19 healthcare activities.
...
The minimum of the total beds in use multiplied by the HCWs per bed and the HCW inpatient cap.
...
Total beds in use multiplied by the number of recommended informal caregivers per hospitalised patient.
...
Minimum of the inpatient cleaner cap and the total beds in use multiplied by the number of hygienists per bed.
...
Total beds in use multiplied by the number of biological engineers per bed. 
...
The sum of tests for mild and moderate patients times the number of informal caregivers by isolating case.
...
The minimum of the total labs multiplied by the number of lab staff per lab and the lab staff cap.
...
The number of tests for HCWs every week is equal to the sum of all of the capped inpatient or lab staff and personnel multiplied by the number of tests per HCW per week as specified in the parameters.
...
The capped number of total tests is the minimum of the uncapped total tests and the max tests per day multiplied by seven.
$$
\begin{align*}
...
Uncapped total tests equals the sum of the capped tests for diagnosis for severe and critical patients, for release of severe and critical patients, the tests for mild, moderate, and suspected cases, in addition to the tests for HCWs and contacts of cases.
