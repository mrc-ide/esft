...
* iso3c - country code, iso3c format
* population - population estimate from the latest available data from the United Nations Development Programme, reported in 2020
* yoy_growth - average year over year growth of country population, estimated from the change in growth in population between UNDP reported numbers in 2015 and 2020
* income_group - World Bank income group
* n_hcws - number of estimated HCWs. Combines latest available estimates of nurses (`Nursing personnel`) and doctors (`Medical doctors`) from WHO GHO with the estimates YoY growth from UNDP to yield estimates of nurses and doctors, the sum of which are the number of healthcare workers. If these are unavailable, uses a combination of income group averages and yoy growth.
* n_labs - number of lab staff, estimated using the same method as the number of HCWs (codes: `Medical and Pathology Laboratory scientists` and `Medical and Pathology Laboratory Technicians`).
* n_hosp_beds - estimated beds per country, calculated using similar procedure as above but with World Bank estimates of hospital beds per 1,000 people.
* perc_beds_crit_covid - percent beds for ICU patients per country, estimated from World Bank/Imperial College Report
* perc_beds_not_covid - percent beds not allocated to COVID, default = 0.4
* perc_beds_sev_covid - percent beds allocated to severe COVID- cases, $1 - (\text{perc_beds_crit_covid} + \text{perc_beds_not_covid})$
* beds_covid - number of hospital beds allocated to COVID $(\text{n_hosp_beds}* (1-\text{perc_beds_not_covid}))$
* severe_beds_covid - number of severe beds allocated to COVID, $\text{n_hosp_beds}*\text{perc_beds_sev_covid}$
* crit_beds_covid - number of critical beds allocated to COVID, $\text{n_hosp_beds}*\text{perc_beds_crit_covid}$
...
* shifts_day - these are the shifts per day specified in function call
* platform_key - snake case version of the testing platform name
* platform - platform name in plain english
* type - type of diagnostics machine
* days_week - days it operates per week
* covid_capacity - capacity of test processing function dedicated to COVID-19
* country_name - country for which this capacity is calculated
* region - WHO region of that country
* income_group - WB income group of that country
* country_code - iso3c code of that country
* modules_activated - number of total modules (a machine can have several modules) in active use in that country
* hours - hours per shift
* throughput_per_day - number of tests that can be processed by that machine per day (with day being specified by combining hours per shift, and shifts per day and cross referencing that with the throughput input data
* total_test_capacity - this is the total number of tests that the different categories of machines can process in a week, depending on the days per week and the throughput per day ($Capacity_{test-total} = Modules_{activated}*Days_{week}*Throughput_{day}$).
* covid_test_capacity - this is the total test capacity multiplied by the covid capacity - or the number of tests that can be run for COVID-19 on the different machines every week ($Capacity_{test-COVID} = Capacity_{test-total}*p_{covid-capacity}$)
...
$$
\begin{align*}
Modules_{sum} = Modules_{roche-6800} + Modules_{roche-8800} + Modules_{abbott-m2000} +\\
Modules_{hologic-panther}+Modules_{hologic-panther-fusion}+Modules_{manual}\\
\\
Labs_{total} = \frac{Modules_{sum}}{3} + \frac{Modules_{genexpert}}{4}
\end{align*}
$$
